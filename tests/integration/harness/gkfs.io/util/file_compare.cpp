...
struct file_compare_options {
    ...
    REFL_DECL_STRUCT(file_compare_options, REFL_DECL_MEMBER(bool, verbose),
...
struct file_compare_output {
    ...
    REFL_DECL_STRUCT(file_compare_output, REFL_DECL_MEMBER(int, retval),
                     REFL_DECL_MEMBER(int, errnum));
};

void
to_json(json& record, const file_compare_output& out) {
    record = serialize(out);
}

int
open_file(const std::string& path, bool verbose) {
...
size_t
read_file(io::buffer& buf, int fd, size_t count) {
    ssize_t rv{};
    size_t total{};
    do {
...
file_compare_init(CLI::App& app) {
    ...
    cmd->add_option("path_1", opts->path_1, "Path to first file")
            ->required()
            ->type_name("");

    cmd->add_option("path_2", opts->path_2, "Path to second file")
            ->required()
            ->type_name("");

    cmd->add_option("count", opts->count,
                    "How many bytes to compare of each file")
            ->required()
            ->type_name("");

    cmd->callback([opts]() { file_compare_exec(*opts); });
}